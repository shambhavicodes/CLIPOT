...
def prepare_data(dataset, data_dir, corruption, batch_size=128, num_workers=1, use_multicrop=False, n_small=4, n_large=2, clip_transforms=None):
    """
    ...
    ----------
    dataset : str
        The name of the dataset to prepare. Should be one of 'cifar10', 'cifar100', 'tiny-imagenet', 'visda', 'PACS', 'office_home', 'VLCS', 'imagenet_r', 'imagenet_a', 'imagenet_v2', 'imagenet_sketch'.
    data_dir : str
    ...
    use_multicrop : bool, optional
        Whether to use multi-crop transform for SwAV. Default is False.
    ...
    """
    if use_multicrop:
        transform = MultiCropTransform(n_small=n_small, n_large=n_large)
    else:
        transform = clip_transforms if clip_transforms is not None else CLIP_TRANSFORMS

    if dataset == 'cifar10':
        loader, classes = prepare_cifar10_data(data_dir, corruption, batch_size=batch_size, num_workers=num_workers)
    elif dataset == 'cifar100':
        loader, classes = prepare_cifar100_data(data_dir, corruption, batch_size=batch_size, num_workers=num_workers)
    elif dataset == 'tiny-imagenet':
        loader, classes = prepare_tinyimagenet(data_dir, corruption, batch_size=batch_size, num_workers=num_workers)
    elif dataset == 'tiny-imagenet':
        loader, classes = prepare_tinyimagenet(data_dir, corruption, batch_size=batch_size, num_workers=num_workers)
    elif dataset in ["visda", "PACS", "office_home", "VLCS", "office31"]:
...
def prepare_cifar10_data(data_dir, corruption, batch_size=128, level=5, size=10000, num_workers=1):
    ...
    if corruption == 'original':
        dataset = torchvision.datasets.CIFAR10(root=data_dir, train=False, download=True, transform=CLIP_TRANSFORMS)
...
def prepare_cifar100_data(data_dir, corruption, batch_size=128, level=5, size=10000, num_workers=1):
    ...
    if corruption == 'original':
        dataset = torchvision.datasets.CIFAR100(root=data_dir, train=False, download=True, transform=CLIP_TRANSFORMS)
...
def prepare_imagefolder_data(data_dir, name, batch_size, num_workers):
    ...
    loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True, drop_last=True)
